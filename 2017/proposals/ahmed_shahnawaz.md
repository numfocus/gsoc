...
#### conda-forge:
Conda is fully open source but due to the fact that it is shipped with Anaconda, it is confused to be maintained by Continuum Analytics. Anaconda was created by Continum and is a distribution including Python and binaries for several open source projects such as Numpy and SciPy. Continum maintains a large number of the Anaconda repositories but conda and Anaconda are completely independent. In fact, conda can be used without ever needing to touch Anaconda repositories using `pip install conda` from PyPI. One can install packages by defining custom channels. Conda has a fully open toolchain and hence it has the potential to be a completely independent package manager maintained by the open source community.
