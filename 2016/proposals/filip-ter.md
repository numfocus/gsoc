...
* Refactor sparse to an independent module: 
	- Change `sparse` to be an independent module named `pandas-sparse` which will be independent of `pandas`
	- This `pandas-sparse` will then be an external dependency of `pandas`, which is how pandas will provide sparse functionality.
	- `pandas-sparse` will have its own test of course, but the `sparse` module already has it's own tests so this should not be too difficult to refactor
...
- Goal 2:
	...
	A new repository `pandas-sparse` will be created, hosted on Github under `pydata`. 
	...
	As mentioned before `pandas-sparse` will be an external dependency of pandas, therefore I will implement conversion methods to pandas that will allow dense data structures to be converted to sparse and vice versa. These methods currently work on the 
	...
	Snippet of what the implementation of the `to_sparse` methods in `pandas/core/series.py` will look like:
	It is virtually the same as before except the import will come from an external project.
	...
	```python
		...
		def to_sparse():
        	...
        	return SparseSeries(self, kind=self.kind,
                	            	   fill_value=self.fill_value)
        
        ...
    ```
   	
	Similarly the `SparseSeries` in `pandas-sparse` will have a `to_dense` method
    ...
    ```python		
		...
		def to_dense():
			...
			return Series(self.sp_values, index=index, name=name)
...
- Goal 3:
	...
	I will make the corresponding entry into whatsnew:
	
	
	in the `doc/source/whatsnew` of `pandas-sparse`:
...
Then make the new module a dependency in `pandas`
    ...
    **Schedule:** 
 	- June 22nd - July 5th:
 		...
 		- Refactor sparse to be its own module
 	- July 6th - July 19th:
 		- Create tests by refactoring the existing ones and if necessary writing new ones.
 ...
 **Achieve Goal 3: Implement enhancements**
 ...
 - Once sparse has been refactored and bugs have been fixed, I can focus on making enhancements to extend the functionality of `sparse`
