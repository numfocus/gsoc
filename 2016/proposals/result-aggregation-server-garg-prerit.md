...
## Abstract 
Software Carpentry has been teaching researchers in science, engineering, medicine, and related disciplines the computing skills they need to get more done in less time and with less pain. Several workshops are held in which students are required to setup their system by installing necessary packages. Currently software carpentry provides the installation-test scripts so that, students can check that they've successfully installed any software required by their workshop. But the results generated by the scripts are not collected anywhere. In this project a server is to be written for storing results of installation-testing scripts, and also enhancing the testing scripts by adding an option for users to collect results and send them to the server.  In this project, an API will be made so that, the results can be collected on the server, and thus can be figured out what all problems students usually face in installations. This will also help in making informed decisions about deprecating older versions of various dependencies or modernizing the lessons to catch up with current systems. 
 
## Technical Details 
I am breaking it down to several steps in which this project has to be approached : 
### System Information collection 
The installation-test script need to be modified to retrieve some useful information about the user and the environment in which he is running the installation-test-script. 
...
``` 
* Compilation environment used by user 
 * Python Version 
 * Python Build 
 * Python Compiler 
 * Python implementation ( e.g. ‘CPython’, ‘IronPython’, ‘Jython’, ‘PyPy’ ) 
  
All of which can be retrieved by the same library "platform" : 
``` 
import platform 
...
``` 
Some or most of these information are being retrieved in the [installation-test-scripts](https://github.com/wking/swc-setup-installation-test). I would just need to collect them. 
### Success/Failure of packages information collection 
* **Successful Installs** list is already provided by the script. A list ```successes``` contains both checker and the version. All I need to do is grab the full name by ```checker.full_name()``` and ```version``` of the successfully installed packages. 
