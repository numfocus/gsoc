...
* *Backend* : **Django**
* *Database* : **PostgreSQL**
* *Frontend* : **Bootstrap** along with **Highcharts** or **Plotly.js**
...
class Workshop(models.Model):
	...
	respondent_ids = models.CharField(max_length=10000)
	json_string = models.TextField()
	url = models.CharField(max_length=500)
	date_conducted = models.DateField()
```	  

The final calculated list of questions for a particular survey data will store JSON objects as: 
```python
[	
	{
		'id': 		question_id,
		'text': 	question_text,
		'type':		question-type # e.g. 'single-choice-radio' or 'matrix-menu'
		'answer':	[
						{
							'id': 		answer_id
							'text':		answer_text
							'count':	answer_count['count']	 	# answer_count shall be user-defined dictionary as described above
						},
					]
	},
]
...
Further, as per the discussions with the mentors, I have been made aware that SQLite is currently being employed in AMY. Thus, we can currently store the JSON as a string in PostgreSQL, to maintain cross-database compatiblity.  
To enable support for simple queries, we will save certain data (e.g. *date of workshop*) in seperate fields in the models.

#### **Frontend**: Bootstrap + Highcharts / Plotly.js
The reason for narrowing down the choices out of various charting libraries to these two is the fact that both these libraries enable developers to design responsive, customizable charts and add functionalities to the already large feature set of *D3.js*, on which they are based.
...
### May 25th -  June 7th
*Begin coding.*  
As I have implemented OAuth during the discussion period and have good experience of coding in Django, I will be developing the backend and use SurveyMonkey OAuth to authenticate the application with Software Carpentry's SurveyMonkey account.
...
The GUI will be designed and the charts and graphs be implemented, using the processed data.
